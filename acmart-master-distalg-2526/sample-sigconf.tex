...
\title{Evaluating Membership and Broadcast Protocols}
...
\authornote{Student number 73954}
\email{nad.martins@campus.fct.unl.pt}
\affiliation{%
  \institution{MEI, DI, FCT, UNL}
}
...
\begin{algorithm}[H]
\caption{Hybrid Gossip Broadcast}
\label{alg:hybrid}
\begin{algorithmic}[1]
...
\State      requested
\State      messageCache
\State      currentEagerFanout
\State      lazyFanout
\State      minEagerFanout
\State      maxEagerFanout
\State      adaptWindow
\State      newCount $\gets 0$
\State      duplicateCount $\gets 0$

\Procedure{BroadcastRequest}{$mid, sender, payload$}
    \State $m \gets (mid, sender, payload)$
    \State \Call{ReceiveFull}{$m, self$}
\EndProcedure

\Procedure{ReceiveFull}{$m, from$}
    \If{$m.mid \notin received$}
        \State $received \gets received \cup \{m.mid\}$
        \State $messageCache[m.mid] \gets m$
        \State $newCount \gets newCount + 1$
        \State Deliver message to application

        \State $eagerTargets \gets$ random subset of neighbours excluding $from$
        \State $lazyTargets \gets$ random subset excluding eagerTargets

        \ForAll{$p \in eagerTargets$}
            \State Send FULL message to $p$
        \EndFor

        \ForAll{$p \in lazyTargets$}
            \State Send IHAVE($m.mid$) to $p$
        \EndFor

    \Else
        \State $duplicateCount \gets duplicateCount + 1$
    \EndIf

    \State \Call{AdaptFanout}{}
\EndProcedure

\Procedure{ReceiveIHave}{$mid, from$}
    \If{$mid \notin received$ \textbf{and} $mid \notin requested$}
        \State $requested \gets requested \cup \{mid\}$
        \State Send REQUEST($mid$) to $from$
    \EndIf
\EndProcedure

\Procedure{ReceiveRequest}{$mid, from$}
    \If{$mid \in messageCache$}
        \State Send FULL($mid$) to $from$
    \EndIf
\EndProcedure

\Procedure{AdaptFanout}{}
    \State $total \gets newCount + duplicateCount$
    \If{$total < adaptWindow$}
        \State \Return
    \EndIf

    \State $dupRate \gets duplicateCount / total$

    \If{$dupRate > 0.5$}
        \State decrease eager fanout
    \ElsIf{$dupRate < 0.2$}
        \State increase eager fanout
    \EndIf

    \State reset counters
\EndProcedure
...
The protocol preserves \emph{at-most-once delivery} because each node keeps
a local set of previously received message identifiers. A message is only
delivered to the application the first time its identifier is observed.
Any subsequent reception of the same full message is classified as a
duplicate and is ignored for delivery purposes.

Assuming the overlay remains connected and messages between correct nodes
are eventually delivered, the protocol also ensures \emph{eventual delivery}
to nodes that are reachable from the broadcaster. This follows from the
fact that, whenever a node receives a message for the first time, it
propagates either the full message to eager targets or an \texttt{IHAVE}
announcement to lazy targets. A node that learns about a message through
an \texttt{IHAVE} and does not yet possess the payload explicitly requests
it through a \texttt{REQUEST} message. Therefore, lazy dissemination does
not suppress delivery; it only postpones full payload transmission until
it is needed.

The adaptive fanout mechanism does not affect safety, since it only changes
the number of eager targets selected in future transmissions and does not
bypass deduplication or local delivery rules. Its impact is therefore on
performance and reliability under particular parameter choices, but not on
the at-most-once delivery property.
...
\subsubsection{Membership Overhead}

Table~\ref{tab:membership} summarizes the membership overhead results in
terms of control messages per second.

\begin{table}[h]
\centering
\caption{Membership overhead (messages per second).}
...
The results show that Cyclon incurs higher membership overhead than
HyParView, with values around 115 msg/s compared to approximately
98–102 msg/s for HyParView. This difference is expected due to the
more frequent shuffle operations in Cyclon (every 2 seconds), while
HyParView performs maintenance less frequently (every 4 seconds).

Additionally, the membership overhead remains consistent across
different broadcast protocols. This confirms that overlay maintenance
is largely independent from the dissemination strategy, as expected
from the modular design of the system.

The missing value for Flood + Cyclon is due to incomplete log data
...
network failures was not explored.
